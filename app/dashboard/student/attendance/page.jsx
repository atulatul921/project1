...
const Attendance = () => {
  ...
  const onDateChange = (date, dateString) => {
    if (date) {
      ...
      API_SINGLETON.get(`/attendance/${shortDate}`)
        .then((result) => {
      ...
      <ConfigProvider theme={{ algorithm: theme.darkAlgorithm }}>
        ...
        <div className="dark w-full p-10 flex flex-col gap-4">
          <div className="flex gap-5 items-center">
            ...
            />
            <Input
              placeholder="Enter the day report..."
              disabled={!selectedBatch}
              value={info.dayReport}
              onChange={(e) =>
                setInfo((oldInfo) => {
                  return {
                    ...oldInfo,
              ...
              style={{ width: 300 }}
              className="rounded-md dark:border-gray-700 dark-login-input-200 dark:text-gray-100 focus:dark:border-violet-400"
            />
          </div>
